...
<xsl:template match="xhtml:p">    
            ...
            <xsl:choose>
                <xsl:when test='self::xhtml:a[@class="footnote-reference"]'>
                    <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/fnref">
                        <Content><xsl:value-of select="substring-before(substring-after(.,'['),']')"/><xsl:if test="position() != last()"><xsl:text> </xsl:text></xsl:if></Content>
                    </CharacterStyleRange>
                </xsl:when>
                <xsl:when test='self::xhtml:span[@class="table_figure"]'>
                    <CharacterStyleRange AppliedCharacterStyle="CharacterStyle/table_figure">
  ...
  <xsl:otherwise>
      <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/pInitial">
        ...
        <Br/>
  </ParagraphStyleRange>  </xsl:otherwise>
  </xsl:choose>  
</xsl:template>
        ...
        <ParagraphStyleRange AppliedParagraphStyle="ParagraphStyle/table">
            <CharacterStyleRange>
                ...
                <Table HeaderRowCount="0" FooterRowCount="0" AppliedTableStyle="TableStyle/$ID/[Basic Table]" TableDirection="LeftToRightDirection">
                    ...
                    <xsl:attribute name="ColumnCount"><xsl:value-of select="count(child::xhtml:tr[3]/xhtml:td)"/></xsl:attribute>
...
<PathPointArray>
<PathPointType Anchor="-{$halfwidth} -{$halfheight}" LeftDirection="-{$halfwidth} -{$halfheight}" RightDirection="-{$halfwidth} -{$halfheight}"/>
<PathPointType Anchor="-{$halfwidth} {$halfheight}" LeftDirection="-{$halfwidth} {$halfheight}" RightDirection="-{$halfwidth} {$halfheight}"/>
<PathPointType Anchor="{$halfwidth} {$halfheight}" LeftDirection="{$halfwidth} {$halfheight}" RightDirection="{$halfwidth} {$halfheight}"/>
<PathPointType Anchor="{$halfwidth} -{$halfheight}" LeftDirection="{$halfwidth} -{$halfheight}" RightDirection="{$halfwidth} -{$halfheight}"/>
</PathPointArray>
